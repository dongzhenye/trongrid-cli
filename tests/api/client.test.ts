...
describe("ApiClient", () => {
  ...
  it("sends GET request to correct mainnet URL", async () => {
    const mockFetch = mock(() =>
      Promise.resolve(new Response(JSON.stringify({ blockID: "abc" })))
    );
  ...
  it("sends POST request with JSON body", async () => {
    const mockFetch = mock(() =>
      Promise.resolve(new Response(JSON.stringify({ balance: 100 })))
    );
    globalThis.fetch = mockFetch;
    ...
    expect(init.body).toBe(
      JSON.stringify({ address: "TXxx", visible: true })
    );
  });
