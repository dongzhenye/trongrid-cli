...
import { saveApiKey, removeApiKey, resolveApiKey } from "../../auth/store.js";
    ...
    .action(async () => {
      ...
      try {
        console.log(
          styleText(
            "dim",
            "Get your API key from https://www.trongrid.io/dashboard",
          ),
        );
  ...
  auth
    ...
    .action(() => {
      ...
      if (key) {
        ...
        console.log(
          styleText("yellow", "Not authenticated. Using free tier (3 QPS)."),
        );
        console.log(
          styleText("dim", 'Run "trongrid auth login" to authenticate.'),
        );
      }
    });
}
