...
- [ ] Centralized address display policy — `truncateAddress` utility exists but "show full vs truncate" decisions are scattered per-renderer. Extract a `formatAddress(addr, context)` that enforces the 6+6 anti-spoofing minimum and "full when space allows" rule from one place. Surfaced during Phase E.
