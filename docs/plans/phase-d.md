...
| **`src/commands/account/transfers.ts`** | **new** — `AccountTransferRow` type (S2 shape with `direction`), `fetchAccountTransfers(client, address, { limit, minTimestamp, maxTimestamp })` via `/v1/accounts/:address/transferrecords`, `TRANSFERS_SORT_CONFIG` with `tieBreakField: "timestamp"`, `registerAccountTransfersCommand(account, parent)` wiring. Render goes through the new `src/output/transfers.ts` `renderCenteredTransferList` from P6b. | M1.1, M1.2, M1.3 |
