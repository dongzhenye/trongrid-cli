...
- v0.1.2 post-mortem (2026-04-20): CI failed on main right after v0.1.2 merge (biome formatter error in `src/output/format.ts` introduced by `8dfde4e`). Three rounds of dual-layer codex review + cognitive self-review did not catch it because lint/format violations are **tool-detectable** — cognitive review can't reliably see them. Root cause: `bun test` (via the repo script) includes build but NOT lint; `prepublishOnly` ran `build && test`, also without lint; self-review checklist was purely cognitive with no mandated tool gate. Shipped fix `c3a8d94` (biome auto-fix, direct-to-main); process hardening in `chore/verify-script`: new `bun run verify` = `lint && build && test` as the single local-CI-parity gate, `prepublishOnly` now runs `verify`, self-review skill updated to require `bun run verify` (exit 0) as tool-gate step before cognitive review.
