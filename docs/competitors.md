...
1. **Commit this doc** as `docs/competitors.md`.
2. **Decide the 4 interlocked questions** together — ideally in one design-review session.
3. **Update `docs/architecture.md`** with the chosen JSON contract for `account tokens` (adding `amount` / `decimals` / `balance_major`).
4. **Update `docs/commands.md`** if command ordering changes or if default-address flag is added.
